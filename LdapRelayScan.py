...
def DoesLdapsCompleteHandshake(dcIp, timeout=5):
    ...
    ssl_sock = ctx.wrap_socket(
        s, suppress_ragged_eofs=False, do_handshake_on_connect=False
    )
    try:
...
if __name__ == "__main__":
    ...
    if (
        options.method == "BOTH"
        and options.u != "guest"
        and (options.p != "defaultpass" or options.nthash != None)
    ):
        ...
        try:
            ...
            if DoesLdapsCompleteHandshake(dc) == True:
                ...
                ldapsChannelBindingWhenSupportedCheck = asyncio.run(
                    run_ldaps_withEPA(username, password, dc, fqdn, options.timeout)
                )
                if (
